...
@objc public protocol FloatingPanelControllerDelegate: NSObjectProtocol {
    // if it returns nil, FloatingPanelController uses the default layout
    @objc optional func floatingPanel(_ vc: FloatingPanelController, layoutFor newCollection: UITraitCollection) -> FloatingPanelLayout?

    // if it returns nil, FloatingPanelController uses the default behavior
    @objc optional func floatingPanel(_ vc: FloatingPanelController, behaviorFor newCollection: UITraitCollection) -> FloatingPanelBehavior?
    ...
    @objc optional func floatingPanelDidChangePosition(_ vc: FloatingPanelController)

    /// Asks the delegate if dragging should begin by the pan gesture recognizer.
    @objc optional func floatingPanelShouldBeginDragging(_ vc: FloatingPanelController) -> Bool

    @objc optional func floatingPanelDidMove(_ vc: FloatingPanelController, currentY: CGFloat) // any surface frame changes in dragging

    // called on start of dragging (may require some time and or distance to move)
    @objc optional func floatingPanelWillBeginDragging(_ vc: FloatingPanelController)
    // called on finger up if the user dragged. velocity is in points/second.
    @objc optional func floatingPanelDidEndDragging(_ vc: FloatingPanelController, withVelocity velocity: CGPoint, targetPosition: FloatingPanelPosition)
    @objc optional func floatingPanelWillBeginDecelerating(_ vc: FloatingPanelController) // called on finger up as we are moving
    @objc optional func floatingPanelDidEndDecelerating(_ vc: FloatingPanelController) // called when scroll view grinds to a halt

    // called on start of dragging to remove its views from a parent view controller
    @objc optional func floatingPanelDidEndDraggingToRemove(_ vc: FloatingPanelController, withVelocity velocity: CGPoint)
    // called when its views are removed from a parent view controller
    @objc optional func floatingPanelDidEndRemove(_ vc: FloatingPanelController)
    ...
    @objc optional func floatingPanel(_ vc: FloatingPanelController, shouldRecognizeSimultaneouslyWith otherGestureRecognizer: UIGestureRecognizer) -> Bool
    ...
    @objc optional func floatingPanel(_ vc: FloatingPanelController, contentOffsetForPinning trackedScrollView: UIScrollView) -> CGPoint
}
//
...
//        return true
//    }
//    func floatingPanelDidMove(_ vc: FloatingPanelController, currentY: CGFloat) {
//
//    }
...
open class FloatingPanelController: UIViewController {
    ...
    deinit {
        print("FloatingPanelController did released.");
    }
