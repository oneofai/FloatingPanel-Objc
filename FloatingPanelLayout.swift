...
@objc public enum FloatingPanelLayoutReference: Int {
    case fromSafeArea = 0
    case fromSuperview = 1
}

@objc public protocol FloatingPanelLayout: NSObjectProtocol {
    ...
    var supportedPositions: Set<Int> { get }
...
public class FloatingPanelDefaultLayout: NSObject, FloatingPanelLayout {
...
public class FloatingPanelDefaultLandscapeLayout: NSObject, FloatingPanelLayout {
    public override init() { super.init() }
...
class FloatingPanelLayoutAdapter: NSObject {
    ...
    var supportedPositions: Set<Int> {
        return layout.supportedPositions
    }

    var validPositions: Set<Int> {
        return supportedPositions.union([FloatingPanelPosition.hidden.rawValue])
    }

    var topMostState: FloatingPanelPosition {
        
        return FloatingPanelPosition(rawValue: supportedPositions.sorted(by: { $0 < $1 }).first ?? FloatingPanelPosition.hidden.rawValue) ?? .hidden
    }

    var bottomMostState: FloatingPanelPosition {
        return FloatingPanelPosition(rawValue: supportedPositions.sorted(by: { $0 < $1 }).last ?? FloatingPanelPosition.hidden.rawValue) ?? .hidden
    }
    ...
    func activateInteractiveLayout(of state: FloatingPanelPosition) {
        ...
        if validPositions.contains(state.rawValue) == false {
    ...
    private func setBackdropAlpha(of target: FloatingPanelPosition) {
        if target == .hidden {
            ...
            self.backdropView.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: layout.backdropAlphaFor(position: target))
        }
    }
    ...
    private func checkLayoutConsistance() {
        ...
        assert(validPositions.contains(layout.initialPosition.rawValue),
               "Does not include an initial position (\(layout.initialPosition)) in (\(validPositions))")

        if layout is FloatingPanelIntrinsicLayout {
            assert(layout.insetFor(position: .full) == 0, "Return `0` for full position on FloatingPanelIntrinsicLayout")
        }
    ...
    func segument(at posY: CGFloat, forward: Bool) -> LayoutSegment {
        ...
        /// |-------|-------|===o===|  |-------|===o===|-------|
        /// pos: o/x, seguement: =
        let sortedPositions = supportedPositions.sorted(by: { $0 < $1 })

        let upperIndex: Int?
        if forward {
            #if swift(>=4.2)
            upperIndex = sortedPositions.firstIndex(where: { posY < positionY(for: FloatingPanelPosition(rawValue: $0) ?? FloatingPanelPosition.full) })
            #else
            upperIndex = sortedPositions.index(where: { posY < positionY(for: FloatingPanelPosition(rawValue: $0) ?? FloatingPanelPosition.full) })
            #endif
        } else {
            #if swift(>=4.2)
            upperIndex = sortedPositions.firstIndex(where: { posY <= positionY(for: FloatingPanelPosition(rawValue: $0) ?? FloatingPanelPosition.full) })
        ...
        switch upperIndex {
        case 0:
            return LayoutSegment(lower: nil, upper: FloatingPanelPosition(rawValue: sortedPositions.first ?? 0) ?? FloatingPanelPosition.full)
        case let upperIndex?:
            return LayoutSegment(lower: FloatingPanelPosition(rawValue: sortedPositions[upperIndex - 1]), upper: FloatingPanelPosition(rawValue: sortedPositions[upperIndex]))
        default:
            return LayoutSegment(lower: FloatingPanelPosition(rawValue: sortedPositions[sortedPositions.endIndex - 1]), upper: nil)
        }
    }
}
