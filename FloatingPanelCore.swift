...
class FloatingPanelCore: NSObject, UIGestureRecognizerDelegate {
    ...
    private var isBottomState: Bool {
        let remains = layoutAdapter.supportedPositions.filter { $0 > state.rawValue }
...
//        backdropView.alpha = 0.0
    ...
    private func move(from: FloatingPanelPosition, to: FloatingPanelPosition, animated: Bool, completion: (() -> Void)? = nil) {
        assert(layoutAdapter.validPositions.contains(to.rawValue), "Can't move to '\(to)' position because it's not valid in the layout")
    ...
    private func panningChange(with translation: CGPoint) {
        ...
        backdropView.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: getBackdropAlpha(at: currentY, with: translation))
        ...
        if let vc = viewcontroller {
            vc.delegate?.floatingPanelDidMove?(vc, currentY: currentY)
        }
    }
    ...
    func targetPosition(from currentY: CGFloat, with velocity: CGPoint) -> (FloatingPanelPosition) {
        ...
        let sortedPositions = Array(supportedPositions).sorted(by: { $0 < $1 }).map { FloatingPanelPosition(rawValue: $0)! }
